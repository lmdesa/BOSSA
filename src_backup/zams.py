...
class EccentricityDistribution:
    ...
    For a given primary of mass m1 and a companion orbit of log10(period) logp, compute the eccentricity probability
    density function (PDF) for that orbit. All orbits with logp <= 0.5 (default) are assumed to be circularized. Orbits
    with logp > 8.0 are not allowed (p=0). Primaries with m1 < 0.8 Msun or m1 > 150.0 Msun are not allowed (p=0).
    ...
    logp_circ : float
        Log10(period) below which all orbits are assumed to be circularized. Should always be greater than logp_min.
    logp_min : float
    ...
    Notes
    -----
    The distribution is by Moe & Di Stefano (2017) [1]_ with small adjustments as described by OUR WORK. It takes the
    ...
    filling fraction be <70% at periastron.

    The minimum, maximum and circularization periods are set as in the original work, with log10 values 0.2, 8.0 and
    0.5, respectively. The minimum m1 is set to 0.8 Msun also in accordance with the original work, but the mass range
    is extended up to 150.0 Msun.
    ...
    def _eta_lowmass(logp):
        """Compute the power law index eta for the given log10(period) and m1 <= 3 Msun."""
        return 0.6 - 0.7 / (logp-0.5)

    @staticmethod
    def _eta_highmass(logp):
        """Compute the power law index eta for the given log10(period) and m1 >= 7 Msun."""
        return 0.9 - 0.2 / (logp-0.5)

    def _set_e_max(self, logp):
        """Set e_max from the condition of <70% Roche lobe filling factor at periastron."""
        p = 10 ** logp
        self.e_max = 1 - (p/2) ** (-2/3)

    def _eta_midmass(self, logp, m1):
        """Compute the power law index eta for the given log10(period) and 3 Msun < m1 < 7 Msun.

        Compute the power law index eta for the given log10(period) and 3 Msun < m1 < 7 Msun. This is given by a linear
        interpolation between the eta functions for late- (<= 3 Msun) and early-type (>= 7 Msun) primaries.
        """
    ...
    def set_parameters(self, m1, logp):
        """Set power law parameters according to the given m1 and log10(period)."""
        self.m1 = m1
        self.logp = logp
        if self.canonical:
            self.eta = 0
    ...
    def prob(self, e):
        ...
        -----
        UserWarning
            If power law parameters are not set up (set_parameters has not been run yet).

        Notes
        -----
        If the set logp is <= the circularization logp (by default, 0.5), forces e=0 by approximating a delta at e=0
        ...
        the cost of slightly shifting the PDF's norm from 1.
        """

        if self.eta is None:
            warnings.warn('Function parameters not set up. Please run set_parameters(m1, logp) first.')
            return
        if self.canonical:
...
class MassRatioDistribution:
    ...
    For a given primary of mass m1 and a companion orbit of log10(period), compute the mass ratio probability density
    ...
    f_twin : float
        Excess fraction of twin pairs (q>0.95) in relation to a pure power law.
    k : float
    ...
    Solar-type primaries are defined as having masses 0.8 Msun < m1 < 1.2 Msun. The midpoint of A-/late B-type primaries
    ...
    intermediary ranges: solar-A (1.2 Msun <= m1 < 3.5 Msun) and A-OB (3.5 Msun < m1 <= 6 Msun).

    The minimum and maximum periods are set as in the original work, with log10 values 0.2 and 8.0, respectively. The
    minimum m1 is set to 0.8 Msun also in accordance with the original work, but the mass range is extended up to 150.0
    Msun. The minimum q is set to 0.1 as in the original work.
    ...
    def _get_logp_twin(m1):
        """Maximum logp of the observed excess twin population for a given m1."""
        if m1 <= 6.5:
            return 8.0 - m1
        else:
            return 1.5
    ...
    def _get_f_twin_logp_large(self, m1, logp):
        ...
        f_twin_logp_large = f_twin_logp_small * (1.0 - (logp-1.0) / (logp_twin-1.0))
    ...
    def _get_gamma_largeq_solar(self, logp):
        """Compute the power law index gamma_largeq (0.3 <= q <= 1.0) for solar-type primaries."""
    ...
    def _get_gamma_largeq_a(self, logp):
        """Compute the power law index gamma_largeq (0.3 <= q <= 1.0) for midpoint A/early B-type primaries."""
        if logp < self.logp_min:
            g = 0.0
        elif logp < 1.0:
            g = -0.5
    ...
    def _get_gamma_largeq_ob(self, logp):
        """Compute the power law index gamma_largeq (0.3 <= q <= 1.0) for mid B, late B and O-type primaries."""
        if logp < self.logp_min:
            g = 0.0
        elif logp < 1.0:
            g = -0.5
    ...
    def _get_gamma_smallq_solar(self, logp):
        """Compute the power law index gamma_smallq (0.1 <= q < 0.3) for solar-type primaries."""
        if logp < self.logp_min:
            g = 0.0
        elif logp <= self.logp_max:
            g = 0.3
    ...
    def _get_gamma_smallq_a(self, logp):
        """Compute the power law index gamma_smallq (0.1 <= q < 0.3) for midpoint A/early B-type primaries."""
        if logp < self.logp_min:
            g = 0.0
        elif logp < 2.5:
            g = 0.2
    ...
    def _get_gamma_smallq_ob(self, logp):
        """Compute the power law index gamma_smallq (0.1 <= q < 0.3) for mid B, late B and O-type primaries."""
        if logp < self.logp_min:
            g = 0
        elif logp < 1.0:
            g = 0.1
    ...
    def _get_gamma_largeq_solar_a(self, m1, logp):
        """Compute the power law index gamma_largeq (0.3 <= q <= 1.0) for primaries between solar and midpoint A types.

        Compute the power law index gamma_largeq (0.3 <= q <= 1.0) for primaries between solar and midpoint A/early B
        types. This is done by interpolating, at the given logp, between the indexes for the two types.
        """
    ...
    def _get_gamma_largeq_a_ob(self, m1, logp):
        """Compute the power law index gamma_largeq (0.3 <= q <= 1.0) for primaries between midpoint A and O/B types.

        Compute the power law index gamma_largeq (0.3 <= q <= 1.0) for primaries between midpoint A/early B and mid B/
        late B/O types. This is done by interpolating, at the given logp, between the indexes for the two types.
        """
    ...
    def _get_gamma_smallq_solar_a(self, m1, logp):
        """Compute the power law index gamma_smallq (0.1 <= q < 0.3) for primaries between solar and midpoint A types.

        Compute the power law index gamma_smallq (0.1 <= q < 0.3) for primaries between solar and midpoint A/early B
        types. This is done by interpolating, at the given logp, between the indexes for the two types.
        """
    ...
    def _get_gamma_smallq_a_ob(self, m1, logp):
        """Compute the power law index gamma_smallq (0.1 <= q < 0.3) for primaries between midpoint A and O/B types.

        Compute the power law index gamma_smallq (0.1 <= q < 0.3) for primaries between midpoint A/early B and mid B/
        late B/O types. This is done by interpolating, at the given logp, between the indexes for the two types.
        """
    ...
    def set_parameters(self, m1, logp):
        """Set power law and twin fraction parameters according to the given m1 and log10(period)."""
        if self.canonical:
...
class CompanionFrequencyDistributionHighQ:
    """Orbital period probability distribution for a ZAMS star pair with 0.3 <= q <= 1.0.

    For a given primary of mass m1, compute the orbital period probability density function (PDF) for a single companion
    with some mass m_cp such that 0.3 <= q <= 1.0 (q=m_cp/m1). The PDF is a strongly m1_table- and log10(period)-
    dependent piecewise function.
    ...
    m1 : float
        Primary mass.
    _a : float
        Slope of f_logp with logp in an interval around logp = 2.7 with half-width _delta_logp.
    _delta_logp : float
        Half-width of the logp interval over which the slope _a is defined.
    logp_min : float
        Minimum allowed log10(period).
    logp_max : float
        Maximum allowed log10(period).
    ...
    companion_frequency_q03(logp)
        For a given log10(period), compute the companion frequency for a primary with the set m1_table.

    Notes
    -----
    The distribution is by Moe & Di Stefano (2017) [1]_, with small adjustments as described by OUR WORK. Although we
    refer to it as a PDF, the distribution is rigorously defined as a frequency of companions with period logp for
    primaries of mass m1,

    .. math::

        f_{\log P; q>0.3} (M_1,P) := \\frac{d N_{cp, q>0.3} }{d N_1\, d\log P},

    i.e., the number of companions, per primary with mass M1, per orbital period decade, around a period P.

    The companion frequency is empirically fitted for 0.2 <= logp < 1 , logp = 2.7, logp = 5.5 and 5.5 < logp <= 8.0.
    For the intermediate intervals [1,2.7) and (2.7,5.5), it is set to increase linearly with logp.
    ...
    def __init__(self, m1):
        ...
        self.m1_min = 0.8
        self.m1_max = 150
        self._logp_thresholds = [self.logp_min, 1.0, 2.7 - self._delta_logp, 2.7 + self._delta_logp, 5.5, self.logp_max]
    ...
    def f_logp1_q03(self):
        ...
        if self._f_logp1_q03 is None:
            self._f_logp1_q03 = 0.02 + 0.04 * np.log10(self.m1) + 0.07 * np.log10(self.m1) ** 2
        return self._f_logp1_q03

    @property
    def f_logp27_q03(self):
        """Frequency of companions with logp = 2.7 and 0.3 <= q <= 1.0 for primaries of mass m1"""
        if self._f_logp27_q03 is None:
            self._f_logp27_q03 = 0.039 + 0.07 * np.log10(self.m1) + 0.01 * np.log10(self.m1) ** 2
    ...
    def f_logp55_q03(self):
        ...
        if self._f_logp55_q03 is None:
            self._f_logp55_q03 = 0.078 - 0.05 * np.log10(self.m1) + 0.04 * np.log10(self.m1) ** 2
    ...
    def _f3(self, logp):
        """Companion frequency for 2.7 - _delta_logp <= logp < 2.7 + _delta_logp."""
    ...
    def companion_frequency_q03(self, logp):
        """Companion frequency for 0.3 <= q <= 1.0 pairs with a given log10(period)."""
...
class CompanionFrequencyDistribution(CompanionFrequencyDistributionHighQ):
    """"Orbital period probability distribution for a ZAMS star pair with 0.1 <= q <= 1.0.
    ...
    q_distr : MassRatioDistribution object
        Mass ratio distribution for a ZAMS star pair with the same primary mass m1.
    n_q03 : float
    ...
    Notes
    -----
    The distribution is by Moe & Di Stefano (2017) [1]_. Most of the observational techniques considered in that work
    are not able to probe pairs below q=0.3, and thus the period distribution is only empirically fitted to the q>0.3
    region, yielding the distribution from class CompanionFrequencyDistributionHighQ.

    However, from the particular observations that do probe 0.1 <= q < 0.3 region, they are able to empirically fit the
    ...
    between pairs above and below q=0.3.

    This class calculates that ratio, and uses it as a correcting factor, as done by the original authors, to turn the
    ...
    See Also
    --------
    CompanionFrequencyDistributionHighQ : distribution from which this one is computed
    """

    def __init__(self, q_distr, m1, canonical=False, extrapolate_canonical_distribution=False):
        """
        Parameters
        ----------
        q_distr : MassRatioDistribution object
            Mass ratio distribution for a ZAMS star pair with the same primary mass m1_table.
        m1 : float
            Primary mass.
        """
        ...
        self.n_q01 = None
        self.canonical = canonical
        self.extrapolate_canonical_distribution = extrapolate_canonical_distribution
    ...
    def _set_n_q01(self):
        ...
        continuity_factor = 0.3 ** (self.q_distr.gamma_largeq - self.q_distr.gamma_smallq)
        self.n_q01 = self.n_q03 + continuity_factor * a

    def companion_frequency_q01(self, logp):
        """Companion frequency for 0.1 <= q <= 1.0 pairs with a given log10(period)."""
        if self.canonical:
...
class ZAMSSystemGenerator:
    """Sample ZAMS multiple systems of arbitrary order, given a primary mass m1, while keeping track of available m1.

    For a primary companion of mass 150 Msun >= m1 >= 0.8 Msun, draws an arbitrary number n_comp of zero-age main
    ...
    initial mass function (PowerLawIMF) that the sample should reproduce.

    Attributes
    ----------
    pairs_table_path : path_like object
        Path to the h5 file containing equiprobable (m1,logp,q,e) sets according to the distributions in this class.
    imf_array : numpy array
        Array of masses from which all component masses will be initially drawn.
    qe_max_tries : int
        Number of attempts to draw a valid q,e pair for a chosen m1,logp before m1 is redrawn.
    dmcomp_tol : float
        Maximum fractional difference between a drawn mass and the closest mass in imf_array for it to be accepted.
    pairs_table : PyTables File
        Table loaded from the file in pairs_table_path, from which all star pairs are taken.
    ...
    m1array_i : int
        Index of the primary mass currently drawn from highmass_imf_array.
    m1_array : float32
    ...
    m1group : PyTables Group
        Table of equiprobable companion orbits for m1choice, identified by a set (logp,q,e).
    ...
    -----
    UserWarning
        If a star system fails to be generated within the set mass constraints.

    Notes
    -----
    This class was built with current population synthesis codes in mind, which do not evolve triples or higher-order
    ...
    such as when computing a volumetric merger rate from the single population merger rate.

    A choice can be made to still evolve the inner pair as an isolated binary anyway, while only accounting for the
    ...
    uncertainties with regard to evolution.

    Currently the sample_system method always returns the inner pair. The user can then choose to evolve it as an
    isolated binary or not, based on the actual number of companions it represents, but not base on the orbital period
    of the outer companions. This option may be implemented in the future.

    Operationally, all n_comp periods are drawn at the start, then their respective q_choice,e_choice pairs are drawn
    ...
    until a succesful pair is found, or the number of tries reaches qe_max_tries.

    If at any point a primary-companion pair fails to be found, the whole system is discarded and an empty list and zero
    mass are returned. Otherwise, the parameters for the inner pair and the total system mass are returned, and the
    component masses are removed from the imf_array arrays.

    Ultimately, all (m1,logp,q,e) parameter sets are taken from the pairs_table h5 file, meant to be opened with the
    PyTables package. The File is structured in 200 Groups, corresponding to 200 equiprobable m1 values drawn from a
    Salpeter PowerLawIMF [1]_. Each Group is structure in 100 Tables, each corresponding to one of 100 equiprobable logp values
    ...
    EccentricityDistribution classes. The orbital parameter distributions are due to Moe & Di Stefano (2017) [2]_.

    This class can be employed on its own to generate individual systems. Its implementation for the generation of an
    entire sample of binaries is handled by the SimpleBinaryPopulation class in the sampling module.

    References
    ----------
    .. [1] Salpeter, E. E. (1955). The Luminosity Function and Stellar Evolution. ApJ, 121, 161. doi:10.1086/145971
    .. [2] Moe, M., Di Stefano, R. (2017). Mind Your Ps and Qs: The Interrelation between Period (P) and Mass-ratio (Q)
        Distributions of Binary Stars. ApJS, 230(2), 55. doi:10.3847/1538-4365/aa6fb6
    ...
    def __init__(self, pairs_table_path, imf_array, qe_max_tries=1, dmcomp_tol=0.05, parent_logger=None):
        """
        Parameters
        ----------
        pairs_table_path : path_like object
            Path to the h5 containing equiprobable (m1_table,logp,q,e) sets from the distributions in this class.
        imf_array : numpy array
            Array of masses from which all component masses will be initially drawn.
        qe_max_tries : int, default : 1
            Number of attempts to draw a valid q,e pair for a chosen m1_table,logp before m1_table is redrawn.
    ...
    def _set_m1choice(self, m1choice_i):
        """Set the m1array_i and m1_array attributes from a given index."""
        self.m1array_i = m1choice_i
        self.m1_array = self.highmass_imf_array[m1choice_i]

    def _set_m1_options(self):
        """Load list of m1_table options and their respective PyTable Group titles from pairs_table."""
    ...
    def _get_m1(self):
        """Get the closest m1_table to m1_array, and its respective PyTable Group title in pairs_table."""
    ...
    def setup_sampler(self):
        """Set up the lowmass and highmass _imf_array attributes, initial m1array_n and load pairs_table."""
    ...
    def open_m1group(self, m1choice_i):
        """Load the PyTables Group object closest to m1_table corresponding to a m1array_i into the m1group attribute."""
    ...
    def sample_system(self, ncomp=1, ncomp_max=1):
        """Sample pairs m1_table, mcomp_array pairs building a multiple system with ncomp >= 0 companions.

        This class is meant to return only the innermost pair or inner binary to the user, while keeping track of the
        ...
        have their masses accounted for, but only the orbital parameters of the innermost pair are stored.

        Parameters
        ----------
        ncomp : int, default : 1
            Number of companions to the primary. Can be 0, signaling an isolated star.
        ncomp_max : int, default : 1
            Maximum number of companions in the overall sample, possibly greater than that for the system being sampled.
        ...
        system_mass : float
            Total mass of the system, including the innermost pair as well as outer companions.
        ...
        Notes
        -----
        All ncomp periods are drawn at the start, then their respective q_table,e_table pairs are drawn from the
        ...
        redrawn until a succesful pair is found, or the number of tries reaches qe_max_tries.

        If at any point a primary-companion pair fails to be found, the whole system is discarded and an empty list and
        zero mass are returned. Otherwise, the parameters for the inner pair and the total system mass are returned, and
        the component masses are removed from the imf_array arrays.
        """

        if ncomp > (len(self.lowmass_imf_array) + len(self.highmass_imf_array[:self.m1array_i])):
            return np.empty(0) #, 0

        system_mass = self.m1_table  # system mass starts with the primary mass
        outer_pairs = np.zeros(4*(ncomp_max-1), np.float32)
        sampled_pairs = np.zeros(12, np.float32)
        logp_i_list = sorted([str(i) for i in np.random.randint(0, 100, ncomp)])  # draw all companion periods
        # periods are drawn as indexes to the Tables within the m1group Group

        lowmcomp_i_list = []  # mass index of < 0.8 Msun companions
        highmcomp_i_list = []  # mass index of >= 0.8 Msun companions
        success = True  # defaults to a success for isolated stars, i.e., ncomp=0
        for order, logp_i in list(enumerate(logp_i_list))[::-1]:  # start sampling from the innermost pair
            logp_table = self.m1group[logp_i]  # open the drawn Table by its index
            logp = np.float32(logp_table._v_title)  # get the corresponding logp
            ...
            while try_number < self.qe_max_tries:
                ...
                # Look for the mass closest to mcomp_array in the relevant imf_array.
                # Checks in place to avoid mass repetition.
                if low_mcomp:
                    if (len(self.lowmass_imf_array) - len(lowmcomp_i_list)) < 1:
                        mcomp_array = 0.9 * mcomp_table / (self.dmcomp_tol + 1)  # no <= 0.8 mcomp available
                    else:
                        mcomp_i = np.searchsorted(self.lowmass_imf_array, mcomp_table, side='left')  # find closest m
                        if mcomp_i == self.lowmass_imf_array.shape[0]:  # avoid out-of-bounds index
                            mcomp_i -= 1
                        if mcomp_i in lowmcomp_i_list:  # avoid repetition
                            mcomp_i -= 1
                    ...
                    if (len(self.highmass_imf_array[:self.m1array_i]) - len(highmcomp_i_list)) < 1:
                        ...
                        # Because m1array is slightly different from m1table, sometimes a q equal to or close to 1 will
                        # result in mcomp_table <= m1table but m1array, which violates our definition of q. The closest
                        # valid mcomp_array is thus the closest value in the mass array to mcomp_table below m1array.
                        mcomp_i = np.searchsorted(self.highmass_imf_array[:self.m1array_i], mcomp_table, side='left')
                        if mcomp_i == self.highmass_imf_array[:self.m1array_i].shape[0]:  # avoid out-of-bounds index
                            mcomp_i -= 1
                        #if mcomp_i == self.m1array_i:  # avoid mcomp_array = m1_table
                        #    mcomp_i -= 1
                        while mcomp_i in highmcomp_i_list:  # avoid repetition
                            mcomp_i -= 1
                        mcomp_array = self.highmass_imf_array[mcomp_i]

                # Check whether the mass drawn from pairs_table and the closest mass in imf_array are sufficiently
                ...
                if dmcomp <= self.dmcomp_tol:
                    ...
                    # Keep track of already drawn masses to avoid repetition.
                    if low_mcomp:
                    ...
                    if order == 0:
                        inner_pair = np.array([self.m1_table,  # m1_table closest to m1_array, taken from pairs_table
                                                self.m1_array,  # m1_table drawn from imf_array
                                                self.dm1,  # relative difference between m1_table and m1_array
                                                mcomp_table,  # mcomp_array drawn from pairs_table
                                                mcomp_array,  # comp closest to mcomp_table, taken from imf_array
                                                dmcomp,  # relative difference between mcomp_array and mcomp_table
                                                q_table,  # mass ratio between table masses
                                                mcomp_array/self.m1_array,  # mass ratio between array masses
                                                logp,  # log10(orbital period)
                                                e_table,  # eccentricity drawn from pairs_table
                                                ncomp,  # actual number of companions corresponding to the system
                                                system_mass  # total system mass considering all companions
                                                ])
                    ...
                    try_number = self.qe_max_tries  # automatically concludes the loop if successful
                else:
                    try_number += 1
            if not success:
                break

        # Once a system has been built successfully, we remove all component masses from the imf_array.
        if success:
            ...
            for i in highmcomp_i_list:
                if i == len(self.highmass_imf_array):
                    print('WE ARE OUT OF BOUNDS')
                    print(self.highmass_imf_array.shape, self.highmass_imf_array)
                    print(highmcomp_i_list)
                    print(self.m1array_i)
                    raise
            try:
            ...
            # A success with a zero system mass in inner_pair signals an isolated star with mass m1_table.
            ...
            #self.logger.debug('Failed to build a valid system within the allowed number of attempts. Discarding...')
            return np.empty(0) #, 0
