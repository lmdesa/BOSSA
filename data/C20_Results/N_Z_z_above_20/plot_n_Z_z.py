...
# this python script allows to visualize the data (plot the SFRD(Z,z) 
# or N(M,M+dM)(Z,z) distribution and CDF at a given redshift)
# usage: python plot_n_Z_z.py
# adjust the data input file name (input_file); see example use at the bottom
# requires 'Time_redshift_deltaT.dat' and input file with the data for a chosen variation (any of the '*FOH_z_dN*.dat')
'''
...
def plot_data_single_panel(input_file, zmin=0.,zmax=10,solar_Z_scale='AndersGrevesse89'):
    ...
    cb.set_label(label=r'$\rm \frac{n(\Delta M_{*})}{\Delta z \Delta Z_{O/H}} [1/Mpc^{3}yr]$',\
                size=labelsize+2, labelpad=13)
...
def plot_CDF(input_file, zzi=[0], color='g',label='' ):
    ...
    ax1.set_ylabel('fraction at Z$_{O/H}<x$',fontsize=labelsize)
    ax1.tick_params(axis='x', which='major', labelsize=ticklabsize)
    ax1.tick_params(axis='y', which='major', labelsize=ticklabsize)
    ax1.set_ylim([0.,1.05])
    ax1.set_xlim([6.,10])
...
input_file='302w14vIMF3aeh_FOH_z_dN_above_20.dat'
...
def main():
    ...
    input_file='302w14vIMF2aeh_FOH_z_dN_above_20.dat'
    image,redshift,delt = get_plot_data(input_file,zmin=0,zmax=10)
